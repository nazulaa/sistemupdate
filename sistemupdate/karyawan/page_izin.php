...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // Siapkan dan eksekusi pernyataan INSERT
    $stmt = $conn->prepare("INSERT INTO izin (nama, role, divisi, tanggal, keterangan) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("sssss", $nama, $role, $divisi, $tanggal, $keterangan);

    if ($stmt->execute()) {
        $message = "Pengajuan izin berhasil ditambahkan!";
    } else {
        $message = "Gagal menambahkan izin: " . $stmt->error;
    }
...
<?php if (!empty($message)): ?>
    <div class="alert mt-3 alert-success"><?php echo $message; ?></div>
<?php endif; ?>
