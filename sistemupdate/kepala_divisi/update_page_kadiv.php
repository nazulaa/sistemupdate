...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($id > 0) {
        $query = "UPDATE updates SET tanggal=?, jam=?, status=?, deskripsi=?, divisi=?, bukti_gambar=? WHERE id=?";
        ...
        // Bind parameter
        mysqli_stmt_bind_param($stmt, "ssssssi", $tanggal, $jam, $status, $deskripsi, $divisi, $bukti_gambar, $id);
    } else {
        $query = "INSERT INTO updates (tanggal, jam, status, deskripsi, divisi, bukti_gambar) VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = mysqli_prepare($conn, $query);
        if (!$stmt) {
            echo "Gagal mempersiapkan pernyataan SQL: " . mysqli_error($conn);
            exit();
    ...
    if (mysqli_stmt_execute($stmt)) {
        header("Location: update_page_kadiv.php");
        exit();
    } else {
...
<div class="table-responsive">
    ...
    <div class="modal fade" id="modalUpdate" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <form action="update_page_kadiv.php" method="POST" enctype="multipart/form-data" class="modal-content">
...
<script>
    ...
    function confirmDelete(id) {
        document.getElementById('confirmDeleteBtn').href = "update_page_kadiv.php?hapus=" + id;
